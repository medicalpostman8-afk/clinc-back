<div>
    ...
    <div class="relative">
        <textarea
            {{ $attributes->only(['placeholder', 'required', 'wire:model'])->merge(['name' => isset($locale) ? "{$name}[{$locale}]" : $name]) }}
            class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 @error(isset($locale) ? "{$name}.{$locale}" : $name) input-error @else @if (isset($locale) ? old("{$name}.{$locale}") : old($name)) input-success @endif @enderror"
            id="{{ $id ?? $name }}"
            rows="{{ $rows ?? 4 }}">{{ isset($locale) ? old("{$name}.{$locale}", $value ?? null) : old($name, $value ?? null) }}</textarea>
