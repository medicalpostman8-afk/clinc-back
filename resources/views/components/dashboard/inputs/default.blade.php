<div>
    ...
    <div class="relative">
        <input
            {{ $attributes->only(['placeholder', 'required', 'wire:model', 'min', 'step'])->merge(['name' => isset($locale) ? "{$name}[{$locale}]" : $name]) }}
    ...
    @isset($description)
        <p class="mt-2 text-xs text-gray-400 dark:text-gray-500">{{ $description }}</p>
    @endisset
